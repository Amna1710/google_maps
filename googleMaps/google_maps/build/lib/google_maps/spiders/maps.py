...
class MapsSpider(scrapy.Spider):
        ...
        # def read_data(self):
        #     with open(
        #             r'D:\PycharmProjects\google_business_vanhaafthen\google_business_vanhaafthen\outputs\HVAC_Companies_Data111.csv',
        #             'r', encoding='utf-8') as file:
        #         reader = list(csv.DictReader(file))
        #         data_list = []
        #         for data in reader:
        #             if f"{data.get('Company Name')} {data.get('Company Address')}" not in data_list:
        #                 data_list.append(f"{data.get('Company Name')} {data.get('Company Address')}")
        #
        #     return data_list
